//August 22, 2019
// This code is using to check the result after each matrix tune from the par_opt.cc
// This code can also be used for the paramater optimization by using the Lambda and Sigma known masses
// 
...
//const double mp = 2.808944; // for tritium
...
void optics(){
  ...
  sprintf(name_angle_phi,"./All_Matrices/PHL_6th_1.dat"); 
  ...
  sprintf(name_Mom_lhrs,"./All_Matrices/LMOM_10th_4.dat");//MOML_2nd_2.dat
 ...
 sprintf(name_Mom_rhrs,"./All_Matrices/MOMR_11th_3.dat");
  ...
  //TH1F *h = new TH1F("h"," ;Missing Mass(MeV/c^{2});Counts ",130,900,1300); 
  ...
  TH1F *h2 = new TH1F("h2"," ; ",130,-200,200); // 130 bin -40 to 40 for back ground analysis
  ...
  for (int i=0 ; i< ent ; i++){
    ...
    //  if(rtrig==true && ((ctime>-39.3765 && ctime< -7.28) ||(ctime>15.0499 && ctime< 39.0487))   && fabs(lvz[0]-rvz[0])<0.045  && fabs(z_av[0])<0.115 ){
      ...
      //    momL[0] = momL[0]*2.21682/2.10; //for H/T and tritium only 
	...
	{delta_pep[0] = -9.95197e-1*sin(-4.66838* ph2[0]) + 1.59176;} 
      ...
      if(z_av[0]>0.08) 
	{delta_pk[0] = 2.55664e-2*ph1[0] + 3.62363;} // make sure which geometry ?????????????????
      else 
	{delta_pk[0] = -9.95197e-1*sin(-4.66838* ph1[0]) + 1.59176;}
      ...
      //  mm = mm -2994.761; // for tritium data only
  ...
  ////  TF1 *f1 = new TF1("f1","gaus",1110.10,1119.56);
...
double CalcMM(double ee, double* par_ep, double* par_k, double mt){
  ...
  Ee = Ee +0.002413;//+0.00736 original value in GEV
