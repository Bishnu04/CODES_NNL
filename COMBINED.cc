// SEPT 12, 2019 copied from par_opt.cc is a matrix tuning code. Take only one data set(H/H or H/T data for tune one time)
// Purpose is to include H/H and H/T for tuning as the H/T doesn't following the H/H kinematics
// On SEPT 15, code is working and can combine bot data H/H and H/T for the tuning process
// Code is working( can take two different data set for tuning matrices... angle tune is not included in this patrt(in the chi2 minimization part). can be easily includedi if needed.)
// In this code I am using Sclaled reconstructed z for HRS mom matrices and RHRS angle matrices. For LHRS angle matriced just reconstructed z is used.
...
const int npeak = 2;
double Lambda_width[npeak] = {4.66, 4.42}; //6.5,8.8}
double Lambda_cent[npeak] ={1115.68,1191.79};
...
const int npeak_2 = 1;
double Lambda_width_2[npeak_2] = {4.0}; //6.5,8.8}
double Lambda_cent_2[npeak_2] ={1115.69};
...
void COMBINED(){
  ...
  t2->Add("./Rootfiles/July22_Rootfiles/H22_T_Aug05.root");// H data with tritium kinematics
  ...
  sprintf(name_Mom_lhrs,"./All_Matrices/MOML_28th_3.dat");
  ifstream Mom_lhrs(name_Mom_lhrs);
  double mom_L[Total_Par];
  for(int i = 0; i<Total_Par;i++){
    double par5 = 0.0;
    int p5 =0;
  ...
  TH1F *h_2 = new TH1F("h_2"," ;Missing Mass(MeV/c^{2});Counts ",150,1025,1275); //H/H 245 and H/T 225
 ...
 bool rtrig_2 = false; 
  ...
  for (int i=0 ; i< ent ; i++){
    ...
    if(rtrig==true &&  fabs(ctime)<1  && fabs(lvz[0]-rvz[0])<0.045  && fabs(z_av[0])<0.115 ){ 
      ...
      //  momL[0] = momL[0]*2.216045/2.10; //for H/T and tritium only 
	...
	{delta_pk[0] =-1.31749*sin(-4.61513* ph1[0]) + 2.03687;}
      ...
      par_ep[0] = pep_real[0] ; // offset includ
      par_ep[1] = th2[0];
      par_ep[2] = ph2[0];
  ...
  for (int i=0 ; i< ent_2 ; i++){
    ...
    if(rtrig_2==true &&  fabs(ctime_2)<1  && fabs(lvz_2[0]-rvz_2[0])<0.045  && fabs(z_av_2[0])<0.115 ){ 
      ...
      ph2_2[0] = ph2_2[0]-hrs_ang;
     

     
     
      momL_2[0] =  calcf2t_th(mom_L, XFP_2, XpFP_2, YFP_2, YpFP_2,  z_av_2[0]); 
      momL_2[0] =momL_2[0]*Momr + Momm; // make sure with TG
    
      momL_2[0] = momL_2[0]*2.21748/2.10; //for H/T and tritium only 
	...
	{delta_pep_2[0] = -1.35758*sin(-4.59571* ph2_2[0]) + 2.09093;} 
      ...
      ph1_2[0] = ph1_2[0] + hrs_ang;  
     
      
      
      momR_2[0] =  calcf2t_th(mom_R, R_XFP_2, R_XpFP_2, R_YFP_2, R_YpFP_2,  z_av_2[0]);
	...
	{delta_pk_2[0] =-1.31749*sin(-4.61513* ph1_2[0]) + 2.03687;}
      ...
      // missing mass calculation==============================
      par_ep_2[0] = pep_real_2[0] ; // offset includ
      par_ep_2[1] = th2_2[0];
      par_ep_2[2] = ph2_2[0];
      
      par_k_2[0] = pk_real_2[0]; // offset included       
      par_k_2[1] = th1_2[0]; 
      par_k_2[2] = ph1_2[0];
      
      hallap_2 = hallap_2-0.148 ;// must be -ve
 ...
 //++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ...
  const int nite =0; // Number of iteration
  double temp[nite]; 
  double x[nite];
  if (nite>0) cout << " Tuning started: " << endl;
  for(int i=0 ; i<nite ; i++){
    x[i] = i+1;
    temp[i] = tune(momR_opt,i); // for RHRS tune 
    sprintf(tempc, "./All_Matrices/MOMR_28th_%d.dat",i);
    //  sprintf(tempc, "./All_Matrices/MOML_X_%d.dat",i);
...
// #############################################################
{
  ...
  for(int i=0 ; i<allparam ; i++){  
    ...
    minuit -> GetParameter(i,momR_opt[i],e); // For RHRS tune
  }
  ...
  
{
  ...
  for(int i=0 ; i<ntune_event ; i++){ 
    ...
    moml[0] =  calcf2t_th(param, XFP, XpFP, YFP, YpFP,  z_av_sc);
    ...
    momr[0] =  calcf2t_th(param, XFP, XpFP, YFP, YpFP,  z_av_sc);
    momr[0] = momr[0]*Momr+Momm;    
    if(z_av>0.08) 
  ...
  for(int i=0 ; i<ntune_event_2; i++){ 
    ...
    // For the LHRS momentum tunning
    moml_2[0] =  calcf2t_th(param, XFP_2, XpFP_2, YFP_2, YpFP_2,  z_av_sc_2);
  ...
  if( z_av_2>0.08)
    ...
    //  for the RHRS momentum tunning
    momr_2[0] =  calcf2t_th(param, XFP_2, XpFP_2, YFP_2, YpFP_2,  z_av_sc_2);
    momr_2[0] = momr_2[0]*Momr+Momm;    
    if(z_av_2>0.08) 
    ...
    chi_22 = chi_22 +8*pow(residual_2,2.0);
