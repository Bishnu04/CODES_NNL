// 11/20/2019  copied from ht_optics.cc
// 
// purpose is to do the Al analysis suggested by Dr. Tang on November 20, 2019
// This is the updated ocde on Dec 6 2019
// This code is based on the Right handed system Thast is th2 = -th2 and ph2 = -ph2 and  vec_ep.RotateX(hrs_ang);
// This code is used to check the histogrsms after the HT_optics_DEC5_19.cc tuned the matrices
...
void Al_analysis(){
  ...
  // t1->Add("./Rootfiles_dummy/D_322_326.root"); 
  //  t1->Add("./Rootfiles_dummy/tritium_111326.root"); 
  ...
  // sprintf(name_Mom_lhrs,"./All_Matrices/mom30_L_3rd_7.dat");
  ...
  // sprintf(name_Mom_lhrs,"./MOM_MATRICES/MomL_5_2nd_0.dat");
  // sprintf(name_Mom_lhrs,"./MOM_MATRICES/mom_LHRS_5_0.dat");// orignal first 5th order matrix Nov 15, 2019
  sprintf(name_Mom_lhrs,"./MOM_MATRICES/MomL5_6th_0.dat");
  ...
  // sprintf(name_Angle_R,"./All_Matrices/theta_R_1st_0.dat");
  ...
  // sprintf(name_phi_Rhrs,"./All_Matrices/phi_R_1st_0.dat");
  ...
  // sprintf(name_Mom_rhrs,"./All_Matrices/mom30_R_5th_3.dat");
  //  sprintf(name_Mom_rhrs,"./All_Matrices/mom7_R_7th_1.dat");
  //  sprintf(name_Mom_rhrs,"./All_Matrices/momRH_4th_7.dat");
  //  sprintf(name_Mom_rhrs,"./MOM_MATRICES/mom_RHRS_5_3rd_2.dat"); // matrix prodeced on the Nov 15, 2019
  // sprintf(name_Mom_rhrs,"./MOM_MATRICES/mom_R5_8th_0.dat"); // matrix prodeced on the Nov 19, 2019
  //  sprintf(name_Mom_rhrs,"./MOM_MATRICES/MomR_5_2nd_0.dat");
  sprintf(name_Mom_rhrs,"./MOM_MATRICES/MomR5_6th_0.dat"); // DEc3 started
  ...
  TH1F *h21 = new TH1F("h21"," ; -B_{#Lambda}(MeV);Counts/1.54 MeV ",162,-100,150); // 232 
  ...
  TH1F *hT = new TH1F("hT","H as target mass in T/T data ; -B_{#Lambda}(MeV);Counts/2.0 MeV ",125,-150,100); // 232 

  TH1F *h36 = new TH1F("h36"," H in T kinematics ; missing mass(MeV/c^{2});Counts/2.5 MeV ",160,900,1300); // to see H in T kinematics
  TH1F *h60 = new TH1F("h60","scaled average z ;",100,-0.3,0.9); 
  TH1F *h61 = new TH1F("h61","unscaled average z  ;",100,-0.3,0.3); 
  ...
  for (int i=0 ; i< ent ; i++){
    ...
    if(fabs(lvz[0]-rvz[0])<0.045 && fabs(z_av[0])<0.10){ // z selection +++++++++++++++++++ z selection ++++++++++++++++
      ...
      momL[0] = momL[0]*2.218/2.10; //for H/T and tritium only 


      par_ep[1] = -th2[0]; // right handed system Dec3 2019
	...
	delta_pk[0] = 3.158e-2 * holiang1 + 4.05819e-1;	
      ...
      hallap = hallap-0.1843;//loss when elctorn entres the target cell must be -ve
      ...
      if(rtrig==true &&  fabs(ctime)<1.0){ /// +++++++++++++++++++++++++++++++++++++++++ coin time  loop++++++++++++++++++++++++++++
       ...
       //  mm = mm -1115.683;  // H as target mass for T/T data
       //	hT->Fill(mm);
    ...
    //  }
      ...
      // // ///// the folloing lines are the original lines before November 14, 2019
      //  if(rtrig==true &&  fabs(ctime)<1  && fabs(lvz[0]-rvz[0])<0.045  && fabs(z_av[0])<0.10){
      // 	 ht->Fill(mm); // Real tritium analysis
      //  }

      //  if(rtrig==true && ((ctime>-39.3765 && ctime< -7.28) ||(ctime>15.0499 && ctime< 39.0487)) && fabs(lvz[0]-rvz[0])<0.045  && fabs(z_av[0]<0.10) ){ 
      // 	hb->Fill(mm); // for back ground analysis
      // }



      // if(rtrig==true &&  fabs(ctime)<1  && fabs(lvz[0]-rvz[0])<0.045  && (z_av[0] < -0.115 ||z_av[0] > 0.09) ){
      // 	h->Fill(mm); // for Al Analysis
      // }
      
      // //  if(rtrig==true && ((ctime>-39.3765 && ctime< -7.28) ||(ctime>15.0499 && ctime< 39.0487)) && fabs(lvz[0]-rvz[0])<0.045  && fabs(z_av[0])<0.115){ 
      // if(rtrig==true && ((ctime>-39.3765 && ctime< -7.28) ||(ctime>15.0499 && ctime< 39.0487)) && fabs(lvz[0]-rvz[0])<0.045  && (z_av[0] < -0.10 ||z_av[0] > 0.10) ){ 
      // 	h2->Fill(mm); // for back ground analysis for the Al spectrum
      // }
  ...
  // //H/H data
  // TF1 *f1 = new TF1("f1","gaus",1110.73,1120.78);
  // TF1 *f2 = new TF1("f2","gaus",1187.93,1197.17);
  ...
  // f1->SetLineWidth(1);
  // f2->SetLineWidth(1);
  // H->Fit("f1","","",1110.73,1120.78);
  // H->Fit("f2","","",1187.93,1197.17);
  // f1->Draw("same"); 
  ...
  // l.DrawLatex(1130,72,Form("#color[2]{mean = %.6g}",f1->GetParameter(1)));
  ...
  // TF1 *f1_2 = new TF1("f1_2","gaus",1110.0,1122.0);
  // TCanvas* c2_2 = new TCanvas("c2_2","c2_2",600,600);
  // c2_2->cd();
  // H1->Draw();
  
  // f1_2->SetLineWidth(1);
  // H1->Fit("f1_2","","",1110.0,1122.0);
  ...
  // // // TCanvas* c3 = new TCanvas("c3","c3",600,600);
  // // // h3->Add(h,h2,1.0,-1.0);
  // // // TH1F *h6 = (TH1F*) h3->Clone(); ///// BP OCT 9
  // // // h3->Draw();
  ...
  // // // //////  TH1F *h6 = (TH1F*) h3->Clone(); ///// BP OCT 9
  // // // h6->Draw();
  // // // h6->Scale(102.0/882.0);
  
  // // // TCanvas* ct = new TCanvas("ct","ct",600,600);
  // // // ct->cd();
  // // // ht->Draw();
  // // // TH1F *h8 = (TH1F*) ht->Clone();
  // // // h6->Draw("E2 same");
  // // // h6->SetFillStyle(3002);
  // // // h6->SetMarkerStyle(28);
  ...
  // // // hb->Scale(1.0/28.0);
  // // // hb->SetMarkerStyle(28);
  // // // hb->SetMarkerColor(2);
  
  // // // TLatex lct;
  // // // lct.SetTextSize(0.025);
  // // // lct.SetTextColor(kGreen);
  // // // lct.DrawLatex(-90,32,Form("background from Al real"));
  // // // lct.SetTextColor(kRed);
  // // // lct.DrawLatex(-90,30,Form("background from accidental peak"));

 
  // // // TCanvas* c6 = new TCanvas("c6","c6",600,600);
  // // // c6->cd();
  // // // //// h8->Draw();
  // // // h4->Add(h6,hb,1.0,1.0);
  // // // h4->Draw();
  // // // TH1F *h9 = (TH1F*) h4->Clone();
  ...
  // // // h8->Draw();
  // // // h9->Draw("E2 same");
  // // // h9->SetFillStyle(3002);
  // // // h9->SetMarkerStyle(28);
  // // // h9->SetMarkerColor(kGreen);
  // // // TLatex l7;
  // // // l7.SetTextSize(0.025);
  // // // l7.SetTextColor(kGreen);
  // // // l7.DrawLatex(-90,32,Form("total background(from Al real + accidental)"));


  // // // TCanvas* cf = new TCanvas("cf","cf",600,600);
  // // // cf->cd();
  // // // hf->Add(h8,h9,1.0,-1.0); ////// was h4 before
  // // // TH1F * h_er = (TH1F*) hf->Clone();
  // // // hf->Draw();
  // // // TLatex lcf;
  // // // lcf.SetTextSize(0.025);
  // // // lcf.DrawLatex(-90,20,Form("background subtracted"));
 
  // // // TCanvas* c_er = new TCanvas("c_er","c_er",600,600);
  // // // c_er->cd();
  // // // h_er->Draw("e");
  // // // TLatex lcer;
  // // // lcer.SetTextSize(0.025);
  // // // lcer.DrawLatex(-90,25,Form("background subtracted"));
  // // //// ++++++++++++++++++++++
  // +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
 ...
 //  // //++++++++++++++++++for new  background analysis on November 14, 2019
  ...
  // TCanvas* c60 =  new TCanvas("c60","c60", 600,600);
  // c60->cd();
  // h60->Draw();

  // TCanvas* c61 =  new TCanvas("c61","c61", 600,600);
  // c61->cd();
  // h61->Draw();
