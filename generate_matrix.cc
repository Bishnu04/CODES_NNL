//October 7, 2019
// This code helps to generate matrix from 4t order(126 parameter) to 5th order matrix (with 252 parameter) or so on. Can bused to prodece from thired order to 4t order matrix.
// Ima using this code to produce the LHRS theta matrix which will  be of the 5th order matrix.
// That is from fourh order angle matrix going to produce the 5th order matrix.
...
void generate_matrix(){ // Probably the nmain function
  // ======================================== //
  // ======= Opening a ROOT file ============ //
  // ======================================== //
 ...
 ifstream *ifs2 = new ifstream("./dat_file/phi_real.dat");
  ...
  // =============================================================
  ntune_event = 0; 
  for(int i=0 ; i<Angle_Par1; i++){ 
    theta_opt[i] = -2222.0;
  }
  char name_Angle_L[500];
  // sprintf(name_Angle_L,"./matrices/theta_3rd_LHRS_Opt_7.dat"); // ++++++++++++++theta_L4_updated.dat  
   sprintf(name_Angle_L,"./tune_matrix/theta_order5_0.dat");
  ifstream Angle_L(name_Angle_L);
  double Theta_L[Angle_Par1];
  ...
  for(int i =0; i<Angle_Par1;i++){
    ...
    Theta_L[i]=par1;
    theta_opt[i] =par1;
     cout<<"the value of thetaL is = :"<<Theta_L[i]<<endl;
      }
  Angle_L.close();
  //===================================
  ...
  sprintf(name_angle_phi,"./phi_matrices/phi_LHRS_3rd_Opt_9.dat"); 
  ...
  TH1F *h = new TH1F("h",";L.tr.tg_th;Counts",500,-0.2,0.2); // 200 March 3
  // TH1F* h = new TH1F("h","Run # 111721;L.tr.tg_ph;counts",100,-0.04,0.04);// 1 D histogram
  TH1F* h1 = new TH1F("h1","Multi-C-foil target",750,-3,3); // -0.5, 0.5
  gStyle->SetOptStat(111111);
  h1->GetXaxis()->SetTitle("z_LHRS(m)");
  h1->GetXaxis()->CenterTitle();
  h1->GetXaxis()->SetRangeUser(-1.2,0.1); // range is  -0.2,0.2
  ...
  for(int i=0 ; i<nfoil ; i++){
    ...
    h2[i]->GetXaxis()->SetRangeUser(-1.2,0.1);
...
// up to here is the Histograms
  ...
  for (int i=0 ; i< ent ; i++){
	...
	if(fcent[j]-selection_width<ztR_wRC[0]
	  ...
	  h3[j]->Fill(th2[0]);//  h3[j]->Fill(ph2[0],th2[0]); 
	  foilfoilflag=true; 
	  foil_with_hit=j;
	}
	...
	if(ntune_event<nmax && row_flag==true){
	  ...
	  beam_y[ntune_event] = TGT_x; /// -ve sign included // ++++
	  ntune_event++;
 ...
 */
  ...
  for(int i=0 ; i<nite ; i++){
    ...
    const int nYpf=5;
    const int nZt=5;
    const int nXt=5; // Note: this is specified to be zero 
    ...
    sprintf(InputFileName,"./phi_matrices/phi_LHRS_3rd_Opt_9.dat");
    ifstream* ifs = new ifstream(InputFileName);
    
    char OutputFileName[500]; // Output file
    sprintf(OutputFileName,"./matrices/phi_L5_%d.dat",i);
		...
		for (a=0;a<n+1;a++){
		  ...
		  if (a+b+c+d+e==n){
		    ...
		    *ofs << par << " "
			 << a << " " << b << " " << c << " "
			 << d << " " << e << endl;
		    //  cout<<"The value of par  is = " <<par<<endl;
		    npar++;
		  }
...
// #############################################################
{
  ...
  for(int i=0 ; i<allparam ; i++){
    ...
    minuit -> GetParameter(i,phi_opt[i],e);
  }
  ...
  
{
  ...
  double XFP, XpFP;
  double YFP, YpFP;
  const double sigma = 0.0045;//  0.0045;
  ...
  for(int i=0 ; i<ntune_event ; i++){
    ...
    //  refz  = phi_real[theta_flag[i]][foil_flag[i]];
